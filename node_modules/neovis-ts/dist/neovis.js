...
var NeoVis = /** @class */ (function () {
    ...
    NeoVis.prototype.buildNodeVisObject = function (n) {
        ...
        var captionKey = this._config && this._config.node && this._config.node.caption, sizeKey = this._config && this._config.node && this._config.node.size, sizeCypher = this._config && this._config.node && this._config.node.sizeCypher, communityKey = this._config && this._config.node && this._config.node.community;
     ...
     * @param formatter
     */
    NeoVis.prototype.setStrategy = function (vs) {
        this._strategy = vs;
    };
    /**
    ...
    NeoVis.prototype.buildEdgeVisObject = function (r) {
        var weightKey = this._config && this._config.relationship && this._config.relationship.thickness, captionKey = this._config && this._config.relationship && this._config.relationship.caption;
        var edge = {};
        ...
        session.run(this._query, { limit: 30 })
                ...
                record.forEach(function (v, k, r) {
                    ...
                    switch (v.constructor.name) {
                        ...
                        case "Path":
                            // console.log("PATH: ", v);
                            var value = v;
                            var n1 = self.buildNodeVisObject(value.start);
                            var n2 = self.buildNodeVisObject(value.end);
                            self._addNode(n1);
                            self._addNode(n2);
                            value.segments.forEach(function (obj) {
                        ...
                        case "Array":
                            v.forEach(function (obj) {
                                ...
                                switch (obj.constructor.name) {
                                    case "Node":
                                        var node_1 = self.buildNodeVisObject(obj);
                                        try {
                                            self._addNode(node_1);
                                        }
                                        catch (e) {
                                            console.error(e);
                                        }
                                        break;
                                    case "Relationship":
                                        var edge_1 = self.buildEdgeVisObject(obj);
                                        try {
                                            self._addEdge(edge_1);
                                        }
                                        catch (e) {
                                            console.error(e);
                                        }
            ...
            onCompleted: function () {
                ...
                self._network.on("selectEdge", function (properties) {
                    var cypher = "MATCH ()-[r]->() WHERE ID(r) IN [" + properties.edges.join(", ") + "] RETURN r";
                    var session = self._driver.session();
                    session.run(cypher)
                        .then(function (results) {
                        // console.log(cypher);
     ...
     * @param neoNodeProps
     */
    NeoVis.prototype.formatNodes = function (neoNodeProps) {
        var _this = this;
        if (!this) {
            return "";
        }
        var map = new Map(Object.entries(neoNodeProps));
        var keys = Array.from(map.keys()).sort();
        var description = new Map();
        keys.map(function (key) {
            var value = map.get(key);
            var keyStrats = _this._strategy.NodeStrategy[key];
            if (keyStrats) {
                keyStrats.map(function (keyStrat) {
                    if (keyStrat.htmlSuffix) {
                        value = typeof (keyStrat.htmlSuffix) === "function" ? keyStrat.htmlSuffix(value) : value;
                    }
                    if (keyStrat.htmlStyling) {
                        var tagAttrs = Object.keys(keyStrat.htmlStyling.htmlAttr).map(function (attr) {
                            var style = keyStrat.htmlStyling.htmlAttr[attr];
                            var styledValue = (typeof style == "string") ? style : style(value);
                            return attr + "=" + '"' + styledValue + '"';
                        }).join(" ");
                        var html = "<strong>" + keyStrat.infoName + ":</strong> " + "<" + keyStrat.htmlStyling.htmlTag + " " + tagAttrs + ">";
        ...
        return Array.from(description.keys()).sort().map(function (key) { return description.get(key); }).join(" <br> ");
    };
    /**
     * Generate HTML description tooltip using strategies
     * @param neoEdgeProps
     */
    NeoVis.prototype.formatEdges = function (neoEdgeProps) {
        var _this = this;
        if (!this) {
            return "";
        }
        var map = new Map(Object.entries(neoEdgeProps));
        var keys = Array.from(map.keys()).sort();
        var description = new Map();
        keys.map(function (key) {
            var value = map.get(key);
            var keyStrats = _this._strategy.EdgeStrategy[key];
            if (keyStrats) {
                keyStrats.map(function (keyStrat) {
                    if (keyStrat.htmlSuffix) {
                        value = typeof (keyStrat.htmlSuffix) === "function" ? keyStrat.htmlSuffix(value) : value;
                    ...
                    if (keyStrat.htmlStyling) {
                        var tagAttrs = Object.keys(keyStrat.htmlStyling.htmlAttr).map(function (attr) {
                            var style = keyStrat.htmlStyling.htmlAttr[attr];
                            var styledValue = (typeof style === "function") ? style(value) : value;
                            return attr + "=" + '"' + styledValue + '"';
                        }).join(" ");
                        var html = "<strong>" + keyStrat.infoName + ":</strong> " + "<" + keyStrat.htmlStyling.htmlTag + " " + tagAttrs + ">";
                        switch (keyStrat.htmlStyling.htmlTag) {
